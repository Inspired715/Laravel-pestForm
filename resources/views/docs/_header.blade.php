<header class="bg-white shadow px-4 mb-8 md:mb-10 py-4">
  <!-- navbar -->
  <nav x-data="{ open: false }" class="max-w-7xl mx-auto flex items-center justify-between relative w-full z-10">
    <a class="flex items-center leading-none" href="{{route('homepage')}}">
      <svg class="fill-current h-8 mr-4 w-8 text-brand-primary-500 sm:h-12 sm:w-12" xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 50 50">
        <path
      ...
      <h1 class="font-bold text-2xl sm:text-3xl text-brand-primary-300">
        PestForm
      </h1>
    </a>
    <div x-on:click="open = !open" class="cursor-pointer flex flex-col justify-between h-4 w-5 gap-0.5 lg:hidden">
      <span class="bg-black h-1 w-full"></span>
      <span class="bg-black h-1 w-full"></span>
      <span class="bg-black h-1 w-full"></span>
    </div>
    <ol
      class="absolute w-full bg-white shadow top-16 z-50 lg:-mx-0 lg:bg-transparent lg:flex lg:items-center lg:mt-0 lg:shadow-none lg:static lg:w-auto lg:font-bold py-5 lg:py-0"
      x-bind:class="open ? '' : 'hidden'">
      <li>
        <a class="block sm:mx-3 md:mx-6 no-underline px-6 sm:px-0 py-5 lg:py-0 text-brand-primary-300 sm:hover:underline"
    ...
    @endif
      <li>
        <a class="block border-t border-black/40 sm:mx-3 md:mx-6 no-underline px-6 sm:px-0 py-5 lg:py-0 text-brand-primary-300 lg:border-t-0 lg:border-transparent sm:hover:underline"
      ...
      @if (Auth::guest())
      <li>
        <a class="block border-t border-black/40 sm:mx-3 md:mx-6 no-underline px-6 lg:px-0 py-5 lg:py-0 text-brand-primary-300 lg:border-t-0 lg:border-transparent lg:hover:underline lg:hidden"
          href="{{ route('register') }}"
        >
          Start Free Trial
        </a>
