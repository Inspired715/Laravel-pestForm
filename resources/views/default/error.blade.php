...
<main class="">
    ...
    <section class="max-w-5xl mx-auto px-4 lg:px-0">
        <div
            class="flex flex-1 items-center justify-center shadow p-10 min-h-[500px]"
            style="background-color: @if($form->submission_succeeded == 'branded') {{ $form->branding_option_4 }} @else 'white' @endif"
        >
            <div class="text-center">
                ...
                @if($form->email_logo ?? '')
                    <div class="mb-4 flex items-center justify-center">
                        <img
                            class="max-w-40 max-h-40 h-auto w-auto object-cover rounded-lg p-1 bg-white border border-gray-darkest"
                            src="{{ $form->email_logo }}"
                            alt="Logo of {{ $form->title }}"
                        >
                    </div>
                @endif

                <h1
                    class="font-normal mb-8 text-2xl"
                    style="color: @if($form->submission_succeeded == 'branded') {{ $form->branding_option_1 }} @else 'blue' @endif"
                >
                    There Was an Error with a File You Uploaded
                </h1>
