...
<body class="bg-brand-gray font-sans pt-14 md:24">
  <div class="absolute bg-brand-primary-200 h-2 left-0 top-0 w-full"></div>

  <main class="px-4 lg:px-0">
    <!-- site logo -->
    <a class="flex items-center justify-center leading-none mb-12" href="/">
      ...
      <h1 class="font-bold text-xl sm:text-2xl text-brand-primary-300">
        PestForm
      </h1>
    </a>
    <div class="bg-white max-w-lg mx-auto rounded shadow-lg w-full">
      <div class="p-10">
        <div class="mb-8 text-center">
          <h1 class="text-brand-primary-400 text-xl">
            Subscribe to <b>{{ $plan->price['product_title'] }}</b>
          </h1>
        </div>
        <div class="space-y-4 mb-4">
{{--          <x-paddle-checkout :override="$link" class="w-full" />--}}
        </div>
      </div>
    </div>
  </main>
...
<script>
    ...
    Paddle.Checkout.open({
        settings: {
            ...
            theme: "light",
            locale: "en",
            successUrl: "{!! route('account.billing') !!}"
        },
        items: itemsList,
        customData: {
            "user_id": '{!! auth()->user()->id !!}',
            "email": '{!! auth()->user()->email !!}',
            "paddle_plan_id": '{!! $plan->id !!}',
            "plan_name": '{!! $plan->internal_name !!}'
        },
    });
</script>
